...
- Do NOT use markdown formatting (no ## headings, no **bold**, no bullet points in the article body)
- Do NOT use hashtags
...
- Do NOT use subheadings with # symbols
- Ensure logical coherence with dynamic rhythm across paragraphs
...
ARTICLE:
[rewritten content as plain text paragraphs only - NO markdown, NO headings, NO bullet points]
"""
        ...
        try:
            ...
            if "HEADLINES:" in text and "ARTICLE:" in text:
                ...
                # Clean any markdown formatting that slipped through
                article = re.sub(r'^#+\s*', '', article, flags=re.MULTILINE)  # Remove # headings
                article = re.sub(r'\*\*([^*]+)\*\*', r'\1', article)  # Remove **bold**
                article = re.sub(r'^\s*[-*]\s+', '', article, flags=re.MULTILINE)  # Remove bullet points
...
with tab1:
    ...
    # Controls
    col1, col2, col3, col4, col5 = st.columns([1.5, 1, 1.5, 1.5, 1])
                    ...
                    with site_cols[idx_site]:
                        ...
                        if is_selected:
                            ...
                            if site_key not in st.session_state.article_rewrites[idx]:
                                ...
                                use_new = st.session_state.use_new_prompt and site_key == 'american_conservatives'
                                style_label = "New Prompt" if use_new else site_config['writer_style']
                                st.caption(f"OK {style_label} style")
